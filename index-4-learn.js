  ...
  if(showExp && _expEmpty){
    ...
    if(isCountType(q)){
      ...
      body+='<div class="exp-opts">'+oArr.map(function(t,i){
        ...
        if(fbCombo && !isAns && oArr.length !== (q.opts||[]).length) return '';   // 폴백: 콤보 파싱 실패 → 정답 보기만. 단 o 칸 수 = 보기 수면 보기별로 쓴 해설이라 다 보인다 [2026-09-26]
