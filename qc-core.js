...
function _qcViolations(q){
  ...
  if(isMCQ){
    ...
    ex.forEach(function(t,i){
      ...
      /* [2026-09-07] 예시가 되묻고 끝나는 자리. 해설이 답을 말했는데 예시가 다시 묻는다.
         길이도 차고 겹치지도 않아 다른 검사를 다 지나간다. 「는지」가 가른다 —
         「…임을 확인한다」는 답을 말한 것이고 「…는지를 확인한다」는 되묻는 것이다.
         잰 값: 공인중개사 76/3,493(2.18%) · 감평·주택관리사 15/10,099(0.15%)
         계산형은 「대입해 가며 확인한다」가 제 자리라 건너뛴다. 막지 않고 경고로만 둔다. */
      if(!_qCalc && _qcOn('gichul','EX_REASK')){ var _rqs=String(t).replace(/<[^>]+>/g,'').trim(); var _rqL=(_rqs.split(/(?<=[.。])\s+/).filter(Boolean).pop()||''); if(/(는지|은지|인지|을지)[^.。]{0,20}(확인한다|확인해\s*본다|살펴본다|따져\s*본다|검토한다|짚어\s*본다)[.。]?$/.test(_rqL) || /문제\s*된다[.。]?$/.test(_rqL)) v.push({kind:'warn',field:'ex',idx:i,code:'EX_REASK',msg:'예시가 되묻고 끝난다 — 해설이 이미 말한 답을 예시가 다시 묻는다. 장면에서 본 것으로 답을 맺으라',text:t}); }
...
var _QC_SEV = {
  ...
  CARD_LT2:'ERROR', CARD_LT2_LINK:'WARNING', REL_NO_ARROW:'WARNING', EX_NONAME:'WARNING', EX_GENERIC_NOUN:'WARNING', EX_PROSE_CALC:'WARNING', EX_REP_VERB:'WARNING', EX_REASK:'WARNING', EX_JOMUN:'WARNING', EX_NO_SUBJECT_FIRST:'WARNING',
