...
/* ===== 🌐 SEO 페이지 생성 (마케팅) — 글루 =====
   생성 로직은 seo-core.js 로 이동(admin.html에서 이 파일보다 먼저 로드).
   여기 남는 것: seoLog(관리자 UI 로그) · seoGenerate(버튼 핸들러: 읽기→생성→zip). */
...
async function seoGenerate(){const btn=document.getElementById('seoBtn');if(btn)btn.disabled=true;const log=document.getElementById('seoLog');if(log)log.textContent='';if(typeof JSZip==='undefined'){seoLog('오류: JSZip 로드 실패(네트워크 확인)');if(btn)btn.disabled=false;return;}try{seoLog('Firestore 읽는 중…');const banks=await seoReadAllBanks(db);let qtot=0;banks.forEach(b=>qtot+=((b.data.questions||[]).length));seoLog('bank '+banks.length+'개 · 문항 '+qtot.toLocaleString()+'개. 페이지 생성 중…');const today=new Date().toISOString().slice(0,10);const zip=new JSZip();const sf=zip.folder('seo');const files=[];const SEO_LABELS={};const pages=new Map();for(const b of banks){const data=b.data;const cert=data.cert,sid=data.subject;const subjName=b.subjName||SEO_SUBJ_OVERRIDE[sid]||data.name||sid;const qs=data.questions||[];const groups={};for(const q of qs){const r=seoRound(q.set)||'__none';(groups[r]=groups[r]||[]).push(q);}for(const r in groups){const rr=(r==='__none')?null:r;const pg=seoPage(cert,subjName,rr,sid,groups[r]);pages.set(pg.fname,pg.html);files.push(pg.fname);SEO_LABELS[pg.fname]=subjName;}}const byCert={};for(const f of files)(byCert[seoCertOfFname(f)]=byCert[seoCertOfFname(f)]||[]).push(f);const hubFiles=[];for(const cert of Object.keys(byCert)){const list=byCert[cert];for(const f of list)sf.file(f,seoAddNav(pages.get(f),cert,f,list,SEO_LABELS));const ch=seoCertHub(cert,list,SEO_LABELS);sf.file(ch.fname,ch.html);hubFiles.push(ch.fname);}const pageUrls=files.map(f=>'https://certlab.ai.kr/seo/'+f);const urls=['https://certlab.ai.kr/seo/index.html'].concat(hubFiles.map(f=>'https://certlab.ai.kr/seo/'+f)).concat(pageUrls);sf.file('index.html',seoHub(['https://certlab.ai.kr/seo/index.html'].concat(pageUrls),SEO_LABELS));zip.file('sitemap.xml',seoSitemap(urls,today));zip.file('robots.txt',seoRobots());zip.file('llms.txt',seoLlms(urls));seoLog('페이지 '+files.length+'장 + 허브·sitemap·robots·llms 생성. 압축 중…');const blob=await zip.generateAsync({type:'blob'});const u=URL.createObjectURL(blob);const a=document.createElement('a');a.href=u;a.download='certlab_seo.zip';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(u);seoLog('✅ 완료 — certlab_seo.zip 다운로드됨. 압축 풀어 GitHub 루트에 업로드(seo/ + sitemap.xml·robots.txt·llms.txt).');}catch(e){seoLog('오류: '+(e&&e.message||e));}if(btn)btn.disabled=false;}
