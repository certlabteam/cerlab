...
/* ============================================================
   [2026-08-05] T3_income_elasticity 흡수
   원본 7_appcode__T3_income_elasticity.js 는 `CertLabITV.T3(mountEl, params)`
   규약(빈 DOM 에 직접 그림)이라 _itvTemplates 의 "HTML 문자열 반환" 규약과 다르다.
   본체는 그대로 두고, 마운트 지점만 만들어 주는 어댑터를 얹는다.
   ============================================================ */
_itvTemplates.T3_income_elasticity=function(it, instId){
  window._itvReg[instId]={template:'T3_income_elasticity', params:(it.params||{}), defaultVal:0};
  var ti=(it.title||it.name)?'<div class="itv-ti">'+_itvEsc(it.title||it.name)+'</div>':'';
  return '<div class="itv-box" id="'+instId+'">'+ti+'<div id="'+instId+'_mt"></div></div>';
};
_itvUpdaters.T3_income_elasticity=function(instId, rawVal, P){
  var m=document.getElementById(instId+'_mt');
  if(!m || m.getAttribute('data-mounted')) return;          // 재렌더 시 두 번 그리지 않는다
  var fn=(window.CertLabITV||{}).T3_income_elasticity;
  if(typeof fn!=='function') return;
  m.setAttribute('data-mounted','1');
  try{ fn(m, P.params); }catch(e){ m.innerHTML='<div class="tbl-broken">⚠️ 인터랙티브 렌더 오류</div>'; }
};
