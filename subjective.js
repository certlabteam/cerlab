...
(function(){
  ...
  function _subjTableHTML(rows){ var maxc=0; rows.forEach(function(r){ if(r.length>maxc)maxc=r.length; });
    ...
    // 넘칠 때 자료박스 안에서 가로 스크롤 — 표 마스터가 쓰는 기존 .tbl-wrap 재사용(새 CSS 없음)
    return '<div class="tbl-wrap"><table class="subj-tbl">'+body+'</table></div>'; }
  ...
  function stpSwap(html,qid){ try{ if(!qid||!SUBJ_TP_MAP[qid]) return html; var a=SUBJ_TP_MAP[qid], h=String(html==null?'':html);
      for(var i=0;i<a.length;i++){ var t=_stpSent(i); if(h.indexOf(t)<0) continue;
        ...
        // 맵 문자열은 그대로 두고 감싸기만 한다 — 표 마스터가 쓰는 기존 .tbl-wrap 재사용
        var tb='<div class="tbl-wrap">'+a[i].html+'</div>';
        h = (h.indexOf(w)>=0) ? h.split(w).join(tb) : h.split(t).join(tb); }
  ...
  function _injectSubjFont(){ if(document.getElementById('subjFontUnify')) return;
    ...
    +'.subj-view .subj-tbl{border-collapse:collapse;width:100%;margin:12px 0;font-size:14px;background:#fff}'
    +'.subj-view .subj-tbl caption{caption-side:top;text-align:left;font-weight:700;padding:0 0 7px;color:#243B53;font-size:14px}'
    +'.subj-view .subj-tbl th,.subj-view .subj-tbl td{border:1px solid #D0D8E2;padding:6px 11px;line-height:1.5;word-break:keep-all}'
    +'.subj-view .subj-tbl th{background:#F2F5F8;text-align:center;font-weight:600;color:#3B4A5A;white-space:nowrap}'
    +'.subj-view .subj-tbl caption ~ tbody td{text-align:right;font-variant-numeric:tabular-nums}'
    +'.subj-view .subj-tbl td:first-child{text-align:center}'
