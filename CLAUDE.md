...
## 2차 주관식 규칙 (2026-08-11 확정)

기준 시험은 **감정평가사 2차**(`appraiser2`, manifest `type: 'subjective'`)다.
「2차」라는 이름만 보고 판단하지 말 것 — 공인중개사 2차(`realestate2`)·주택관리사 2차(`housing2`)는
`type: 'mcq'` 라 여기 해당 없고 1차와 똑같이 다룬다. **판정은 언제나 `type` 으로 한다.**

- **주관식 시험엔 레벨업 전용 문항(variantq)을 만들지 않는다.** 답을 틀리게 쓸 위험이 크다.
  실력이 붙었다고 크리스가 판단할 때까지 유지한다.
- 레벨업은 **기출을 그대로 돌려서** 굴린다. `_lt2SubjPool()` 이 `asks` 있는 은행 문항을 모아
  과목별로 뿌리는 구조이고, `variantPool` 은 주관식 경로에서 아예 안 읽는다. 이대로 둔다.
- 풀이 비면 `_luToast('… 레벨업 문제가 아직 없어요')` 로 조용히 빠진다. 이게 정상 동작이다.
- **2차 문항을 새로 만들거나 고치면 크리스 검토를 받는다.** 자동 승인 금지.
- 검수는 `qc-subjective.js` (`subjectiveGate`). `qc-core.js`·`qc-gate-patch.js` 는 `opts`/`exp.o`
  기준이라 `asks`·`outline` 을 **아예 안 본다** — 2차를 거기 맡기면 전부 통과해 버린다.
